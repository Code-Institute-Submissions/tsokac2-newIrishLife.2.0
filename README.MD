...
**I want to stay up-to-date with all fresh information regarding work and lifestyle, subscribing with my email for newsletter services.**
...
**jQuery** -  **APPLIED jQuery DOM EVENTS IN PROJECT**  - JavaScript library https://code.jquery.com/

**Trip.html**, **Work.html** and **Life.html** are containing Quick Links user cards elements. Every card element contains a Main link source to external services provider and a "Quick Links..." button element for loading detailed links when the user chose the services provider destination.  [Example Screenshot](https://github.com/tsokac2/newIrishLife.2.0/blob/master/assets/wireframes/quicklinks_life.png)
...
* SCRIPTS INTEGRATION:
  ...
  * Creating callback function in `<script src="assets/js/maps.js"></script>`:
  ```
      function initMap() {
...
* SCRIPTS INTEGRATION:
  * Before colosing **`<head>`** element place folowing **CDN** script tag - `<script type="text/javascript"src="https://cdn.jsdelivr.net/npm/emailjs-com@2/dist/email.min.js"></script>`
  * Place following callback function after **CDN** link
...
**ISSUE #8**
Error: 	.section-home .quote p 	Value Error : font-size only 0 can be a unit. You must put a unit after your number : 1.5

SOLUTION: Add unit type after font-size property value `font-size: 1.5rem;`

**ISSUE #9**
Error: .section-home .intro-pitch .pitch-wrap .composition 	Value Error : margin-top  2rem is not a margin-top value :  2rem

SOLUTION: Remove exstra space in `margin-top:  2rem;` to `margin-top: 2rem;`
